...
async function run(){
    ...
    const jobsCollection = await client.db("test").collection("jobs")
    try  {
    ...
    await jobsCollection.insertMany(allParents)
    await jobsCollection.insertMany(allChildren)
    
    console.log(await jobsCollection.countDocuments())
    process.exit()
}
